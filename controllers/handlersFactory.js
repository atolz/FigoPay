...
exports.getAll = (Model, popOption) =>
  catchAsync(async (req, res) => {
    ...
    if (req.trans) filter = { sender: req.params.id };
    ...
    const featuresApi = new APIFeatures(query, req.query).filter().sort().limitFields().pagination();
...
exports.deleteOne = (Model) =>
  catchAsync(async (req, res, next) => {
    ...
    if (!doc) {
      return next(new AppError("No document was found with that ID: No such Document to delete", 404));
    }
